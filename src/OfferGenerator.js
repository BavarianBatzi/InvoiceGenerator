...
const generatePDFContent = (pdf, startYOffset) => {
  const TOP_MARGIN = 10;
  const FOOTER_HEIGHT = 25; // muss zur renderFooter-Geometrie passen
  const LINE_H = 6;
  ...
  const renderHeader = () => {
    // optional: Dokumenttitel/Logo klein etc.
  };
  ...
  // --- Kern: Bricht VOR JEDEM Block um, falls nicht genug Platz ---
  ...
  // --- Hinweiszeile: immer vor Footer prüfen und ggf. umbrechen ---
  ensureSpace(10, { withTableHeader: true });
  pdf.setFontSize(10);
